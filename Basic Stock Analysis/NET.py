...
# visualize financial data for the year of 2020, the distribution of the stock prices for the past year
# Cloudflare's earnings and revenue in the last four quarters, the actual vs. estimated earnings per share for the four quarters in 2020
...
# to get an understanding of the distribution of the Cloudflare quarterly stock prices for 2020. 
...
# Improve the readability of the chart by adding a title of the plot. 
# Add `"Distribution of 2020 Cloudflare Stock Prices by Quarter"` by using `ax.set_title()`
# Change `ylabel` to "Closing Stock Price"
# Change `xlabel` to "Business Quarters in 2020"
...
plt.show()
# seems to be steady growth each quarter followed by high variance and growth in Q4

# chart the performance of the earnings per share (EPS) by graphing the estimate Yahoo projected for the Quarter compared to the actual earnings for that quarters. 
# will accomplish this using a scatter chart. 
# actual EPS by using `x_positions` and `earnings_actual` with the `plt.scatter()` function. Assign `red` as the color.
# actual EPS by using `x_positions` and `earnings_estimate` with the `plt.scatter()` function. Assign `blue` as the color
# estimates and actual EPS are the same. To account for this, set  transparency  `alpha=0.5` to allow for visibility pf overlapping datapoint.
# Add legend by using `plt.legend()` and passing in a list with two strings `["Actual", "Estimate"]`
...
plt.show()
# actuals consistently appear to beat estimates

# visualize the earnings and revenue reported by Cloudflare by mapping two bars side-by-side. 
...
# Create a legend for  bar chart with the `labels` provided
# Add a descriptive title for  chart with `plt.title()`
# Add labels to each quarter by assigning the position of the ticks through the code provided. Hint:  `plt.xticks(middle_x, quarter_labels)`
...
plt.show()
# apparently, while revenue is growing, net income is not

# compare Cloudflare stock to Akamai stock in 2020. We will accomplish this by plotting two line charts side by side in one figure. 
...
plt.show()
# cloudflare seems to really overtake akamai in Q4